...
import disaster.loss.config.EmbeddedKafka;
import disaster.loss.config.KafkaSseConsumer;
import disaster.loss.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class DisasterLossKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/disaster-loss-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/disaster-loss-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/disaster-loss-kafka/unregister"));
                return;
            }
        }
