...
public interface SendaiMonitorAggregateTartgetDRepository extends JpaRepository<Infrastructure, Integer> {
	...
	ISendaiAggregateDTO destroyedOrDamagedHealthFacilities(@Param("from") LocalDate dateFrom,
	...
	@Query(value = "SELECT GREATEST(0, SUM(i.damaged + i.destroyed)) AS totalCount,\n"
			...
			+ " and d.incident_date BETWEEN :from AND :to", nativeQuery = true)
	ISendaiAggregateDTO damagedOrDestroyedEducationalFacilities(@Param("from") LocalDate dateFrom,
			@Param("to") LocalDate dateTo);
	...
	@Query(value = "SELECT GREATEST(0, SUM(i.damaged + i.destroyed)) AS totalCount,\n"
			...
			+ "FROM public.infrastructure AS i inner join disaster AS d on d.disaster_id = i.disaster_id \n"
			+ "where infractructure_type_id in " + "('85f753c8-a495-11ec-b375-90ccdfa85f19')"
			+ " and d.incident_date BETWEEN :from AND :to", nativeQuery = true)
	...
	// D-6 Number of people whose damaged dwellings were attributed to disasters.
	...
	ISendaiAggregateDTO disruptionsToEducationalServices(@Param("from") LocalDate dateFrom,
			@Param("to") LocalDate dateTo);

	// D-7 Number of people whose damaged dwellings were attributed to disasters.
	@Query(value = "SELECT 'name' AS name, SUM(c.value) AS totalCount,\n"
	...
	// D-8 Number of people whose damaged dwellings were attributed to disasters.
	@Query(value = "SELECT 'name' AS name, SUM(c.value) AS totalCount,\n"
			+ "'D-8 Number of disruptions to other basic services attributed to disasters.' AS title\n"
			+ "FROM public.human_population AS c \n" + "where human_population_disaster_category_id in "
			+ "('113b3ae1-a5b8-11ec-adfd-90ccdfa85f11')", nativeQuery = true)
	ISendaiAggregateDTO disruptionsToOtherBasicServices(@Param("from") LocalDate dateFrom,
			@Param("to") LocalDate dateTo);
