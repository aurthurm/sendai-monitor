...
/**
 * REST controller for managing {@link disaster.loss.domain.Address}.
 */
@RestController
@RequestMapping("/api")
public class AddressResource {
    ...
    public ResponseEntity<Address> createAddress(@RequestBody Address address) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/addresses/" + result.getAddressId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getAddressId()))
            .body(result);
    }

    /**
     * {@code PUT  /addresses/:addressId} : Updates an existing address.
     *
     * @param addressId the id of the address to save.
    ...
    public ResponseEntity<Address> updateAddress(
        @PathVariable(value = "addressId", required = false) final String addressId,
        @RequestBody Address address
    ) throws URISyntaxException {
        log.debug("REST request to update Address : {}, {}", addressId, address);
        if (address.getAddressId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(addressId, address.getAddressId())) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, address.getAddressId()))
            .body(result);
    }

    /**
     * {@code PATCH  /addresses/:addressId} : Partial updates given fields of an existing address, field will ignore if it is null
     *
     * @param addressId the id of the address to save.
     * @param address the address to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated address,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/addresses/{addressId}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Address> partialUpdateAddress(
        @PathVariable(value = "addressId", required = false) final String addressId,
        @RequestBody Address address
    ) throws URISyntaxException {
        log.debug("REST request to partial update Address partially : {}, {}", addressId, address);
        if (address.getAddressId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(addressId, address.getAddressId())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, address.getAddressId())
        );
    }
