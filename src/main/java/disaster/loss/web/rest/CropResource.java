...
public class CropResource {
    ...
    public ResponseEntity<Crop> createCrop(@RequestBody Crop crop) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/crops/" + result.getCropId()))
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/crops/{cropId}", consumes = { "application/json", "application/merge-patch+json" })
    ...
    /**
     * {@code GET  /crops/disaster/:disasterId} : get crops for disaster.
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of crops in body.
     */
    @GetMapping("/crops/disaster/{disasterId}")
