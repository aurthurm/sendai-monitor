...
/**
 * REST controller for managing {@link disaster.loss.domain.Village}.
 */
@RestController
@RequestMapping("/api")
public class VillageResource {
    ...
    public ResponseEntity<Village> createVillage(@RequestBody Village village) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/villages/" + result.getVillageId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getVillageId()))
            .body(result);
    }

    /**
     * {@code PUT  /villages/:villageId} : Updates an existing village.
     *
     * @param villageId the id of the village to save.
    ...
    public ResponseEntity<Village> updateVillage(
        @PathVariable(value = "villageId", required = false) final String villageId,
        @RequestBody Village village
    ) throws URISyntaxException {
        log.debug("REST request to update Village : {}, {}", villageId, village);
        if (village.getVillageId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(villageId, village.getVillageId())) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, village.getVillageId()))
            .body(result);
    }

    /**
     * {@code PATCH  /villages/:villageId} : Partial updates given fields of an existing village, field will ignore if it is null
     *
     * @param villageId the id of the village to save.
     * @param village the village to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated village,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/villages/{villageId}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Village> partialUpdateVillage(
        @PathVariable(value = "villageId", required = false) final String villageId,
        @RequestBody Village village
    ) throws URISyntaxException {
        log.debug("REST request to partial update Village partially : {}, {}", villageId, village);
        if (village.getVillageId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(villageId, village.getVillageId())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, village.getVillageId())
        );
    }
     ...
     */
    @GetMapping("/villages")
    public ResponseEntity<List<Village>> getAllVillages(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        log.debug("REST request to get a page of Villages");
        Page<Village> page = villageService.findAll(pageable);
