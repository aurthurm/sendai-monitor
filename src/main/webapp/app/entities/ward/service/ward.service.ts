...
export class WardService {
  ...
  update(ward: IWard): Observable<EntityResponseType> {
    return this.http.put<IWard>(`${this.resourceUrl}/${getWardIdentifier(ward) as string}`, ward, { observe: 'response' });
  }

  partialUpdate(ward: IWard): Observable<EntityResponseType> {
    return this.http.patch<IWard>(`${this.resourceUrl}/${getWardIdentifier(ward) as string}`, ward, { observe: 'response' });
  }
  ...
  findByDistrictId(districtId: string): Observable<EntityArrayResponseType> {
    return this.http.get<IWard[]>(`${this.resourceUrl+'/district'}/${districtId}`, { observe: 'response' });
  }
